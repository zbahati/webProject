...
Class Action {
	...
	function save_record(){
		...
		$chk = $this->db->query("SELECT * FROM tparent where parent_code ='$parent_code' and id != '$id' ");
		if($chk->num_rows > 0){
			return 2;
			exit;
		}
		if(empty($id)){
			$save = $this->db->query("INSERT INTO tparent set $data");
		}else{
			$save = $this->db->query("UPDATE tparent set $data where id = $id");
		}
		if($save){
			return 1;
		}
